...
在保留 Odoo 权限规则的前提下，为 Odoo 数据提供受治理的 MCP 访问能力，支持内置 AI Chat，以及内嵌或外部 Foggy 引擎。
...
- 支持内嵌 Python 引擎或外部 Foggy 服务部署
- 权限解析失败时默认拒绝，避免放行
- 一个面向 Odoo 的可落地 AI 数据治理层

## 部署模式

- **内嵌 Python 引擎**：运行在 Odoo 进程内部
- **外部 Python 引擎**：把请求转发到独立的 Foggy Python 服务
- **外部 Java 引擎**：把请求转发到独立的 Foggy Java 服务

如果你想最快跑通 demo 或较轻量的自托管部署，优先使用内嵌模式。  
如果你希望把查询引擎与 Odoo 隔离部署，可以选择外部 Python 或 Java 模式。
...
|---|---|
| 仅作为独立 MCP gateway 使用，不启用内置 AI Chat | 无 |
...
- 如果你选择网关模式并连接外部 Java 或 Python Foggy 服务，Odoo 插件本身不需要这些 LLM SDK
...
3. **按步骤完成配置**：
   - 复制自动生成的 Docker 命令
   - 启动 Foggy MCP Server（已内置模型）
   - 测试连接
   - 配置数据源（自动填充 Odoo 数据库信息）
   - 初始化闭包表
...
- **Foggy MCP Server**：语义查询引擎，内置 Odoo TM/QM 模型
  - Docker 镜像：`foggysource/foggy-odoo-mcp:v8.1.8-beta`
  - 支持通过 API 动态配置数据源
