...
在保留 Odoo 权限规则的前提下，为 Odoo 数据提供受治理的 MCP 访问能力。

Foggy Odoo Bridge 是一个 Odoo 插件，让 Claude、Cursor 以及其他 MCP 客户端可以查询 Odoo 业务数据，而不会绕过 Odoo 的权限体系。

这个仓库目前作为 Community Edition 基线维护，面向个人用户、开发者和轻量技术评估。它重点提供受治理的 MCP 查询流程、Odoo 权限桥接能力，以及一套适合 Foggy 引擎 adoption 的核心功能。

Foggy Odoo Bridge Pro 是面向商业生产团队的版本，提供 Odoo 内置 governed AI Chat、更丰富的模型包、导出、审计、商业安装体验和支持。
...
- 权限解析失败时默认拒绝，避免放行
- 一条面向 Odoo 的 MCP 与语义查询评估路径

## 依赖说明

Community Edition 不内置 AI Chat。Odoo 环境中不需要安装 `openai` 或 `anthropic`；外部 MCP 客户端的模型/provider 依赖由客户端自身管理。

内嵌语义查询执行需要 Odoo Python 环境安装 `asyncpg`、`pydantic` 和 `PyYAML`。仓库内 Dockerfile 会安装这些运行时依赖，但不会加入 AI provider SDK。

如果是手动安装到已有 Odoo 环境，请先安装运行时依赖，再安装或升级 addon：
...
| 需求 | Community Edition | Foggy Odoo Bridge Pro |
|---|---|---|
| 个人 Odoo + MCP 技术评估 | 支持 | 支持 |
| Claude Desktop / Cursor 等外部 MCP 客户端 | 支持 | 支持 |
| Odoo 内置 AI Chat | 不提供 | 支持 |
| 导出、审计、商业治理工作流 | 不提供 | 支持 |
| 更丰富的商业 Odoo 模型包 | 有限 | 支持 |
| 生产支持和 Odoo Apps 商业体验 | 不提供 | 支持 |

Community 不是功能受限的 Pro 试用版，而是 MCP、TM/QM 和语义查询 adoption 的开放轻量入口。当需求变成商业 Odoo 使用体验、内置 AI 工作流、审计导出和生产支持时，应使用 Pro。
...
对于支持 Streamable HTTP / remote MCP server 的客户端，可使用如下配置：
...
准备 Odoo Apps Community 上架包前，还需要执行：
